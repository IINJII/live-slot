...
function buildCreativeHtml(
  ...
  const style = "width:100%;height:100%;display:block;object-fit:cover;";
...
<div style="position:absolute;bottom:4px;right:4px;background:#000;color:#fff;font-family:monospace;font-size:10px;padding:2px 6px;letter-spacing:0.1em;text-transform:uppercase;">Your Ad</div>`;
  ...
  if (
    creativeMimeType === "application/zip" ||
    creativeMimeType === "application/x-zip-compressed"
  ) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const creativeHtml = buildCreativeHtml(
      creativeBase64,
      creativeMimeType,
      slot.width,
      slot.height,
    ...
    if (!injected) {
      ...
      for (const el of candidates) {
        ...
        if (widthMatch && heightMatch) {
          ...
          if (
            Math.abs(w - slot.width) <= 12 &&
            Math.abs(h - slot.height) <= 12
          ) {
            el.setAttribute(
              "style",
              `position:relative;overflow:hidden;width:${slot.width}px;height:${slot.height}px;` +
    ...
    if (!injected) {
      return NextResponse.json(
        { error: "Could not locate the ad slot element in the page HTML." },
        { status: 422 },
      );
    }
