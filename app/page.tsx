...
export default function Home() {
  ...
  const handleFile = useCallback(async (file: File) => {
    ...
    if (!ACCEPTED_MIME.includes(file.type)) {
      setUploadError('Unsupported format. Accepted: JPG PNG GIF WebP MP4 WebM ZIP');
      return;
    }
  ...
  const clearCreative = () => {
    if (creative) {
      fetch('/api/cleanup', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ fileId: creative.fileId }) }).catch(() => {});
      sessionStorage.removeItem('ls_fileId'); sessionStorage.removeItem('ls_fileName');
      sessionStorage.removeItem('ls_fileType'); sessionStorage.removeItem('ls_mimeType');
      sessionStorage.removeItem('ls_width'); sessionStorage.removeItem('ls_height');
  ...
  const handleDetect = () => {
    ...
    router.push(`/results?fileId=${encodeURIComponent(creative.fileId)}&url=${encodeURIComponent(url)}`);
  };
    ...
    <div className="min-h-screen bg-white flex flex-col">
      ...
      <nav className="border-b border-[var(--line)] px-6 py-4 flex items-center justify-between">
        <div className="flex items-baseline gap-3">
          ...
          <span className="font-mono text-xs text-[var(--text-muted)] tracking-widest uppercase hidden sm:block">Ad Preview Platform</span>
        </div>
        <span className="font-mono text-xs text-[var(--text-dim)] tracking-wider uppercase">Campaign Setup</span>
      </nav>
      ...
      <main className="flex-1 flex flex-col justify-center px-6 py-16 max-w-5xl mx-auto w-full">
        ...
        <div className="mb-16 animate-fade-up">
          <p className="font-mono text-xs tracking-[0.25em] uppercase text-[var(--text-muted)] mb-4">Step 1 of 2</p>
          <h1 className="font-serif text-6xl sm:text-7xl text-black leading-[1.05] mb-6">
            Set up your<br /><span className="italic">campaign preview.</span>
          </h1>
          <p className="font-sans-ui text-lg text-[var(--text-secondary)] max-w-xl leading-relaxed">
            Upload your ad creative and enter the URL of the website you want to preview it on. We&apos;ll find every available ad slot automatically.
          </p>
        </div>

        {/* Two-column form */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-0 border border-black animate-fade-up" style={{ animationDelay: '0.1s' }}>

          {/* Left: Upload */}
          <div className="border-b lg:border-b-0 lg:border-r border-black p-8">
            <div className="mb-6">
              <p className="font-mono text-xs tracking-widest uppercase text-[var(--text-muted)] mb-1">Creative</p>
              <p className="font-sans-ui text-xl font-600 text-black">Upload your ad file</p>
            </div>
                ...
                <div className="border border-[var(--line)] p-4 flex items-start gap-4 mb-4">
                  ...
                  <div className="w-16 h-16 shrink-0 bg-[var(--surface-2)] border border-[var(--line)] overflow-hidden flex items-center justify-center">
                    {(creative.fileType === 'image' || creative.fileType === 'gif') ? (
                      ...
                      <span className="font-mono text-xs text-[var(--text-muted)]">ZIP</span>
                    )}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="font-sans-ui text-base font-600 text-black truncate">{creative.fileName}</p>
                    <div className="flex flex-wrap gap-3 mt-1.5">
                      <span className="font-mono text-xs uppercase tracking-wider text-[var(--text-muted)]">{creative.fileType}</span>
                      {creative.width > 0 && <span className="font-mono text-xs text-[var(--text-muted)]">{creative.width}×{creative.height}</span>}
                      <span className="font-mono text-xs text-[var(--text-muted)]">{formatBytes(creative.size)}</span>
                    </div>
                  </div>
                  <button onClick={clearCreative} className="shrink-0 text-[var(--text-muted)] hover:text-black transition-colors p-1" title="Remove">
              ...
              <div>
                <div
                  ...
                  className={`border-2 border-dashed cursor-pointer flex flex-col items-center justify-center gap-4 py-12 px-8 text-center transition-colors bg-[var(--surface-1)] ${
                    isDragging ? 'border-black bg-[var(--surface-2)]' : 'border-black/30 hover:border-black hover:bg-[var(--surface-2)]'
                  }`}
                >
                  {isUploading ? (
                    <>
                      <div className="w-8 h-8 border border-black border-t-transparent animate-spin" style={{ borderRadius: 0 }} />
                      <span className="font-mono text-sm text-[var(--text-muted)] tracking-wider uppercase">Uploading…</span>
                    </>
                  ) : (
                    <>
                      ...
                      <div>
                        <p className="font-sans-ui text-base text-black font-500">Drop file here or click to browse</p>
                        <p className="font-mono text-xs text-[var(--text-muted)] mt-2 tracking-wider uppercase">JPG · PNG · GIF · WebP · MP4 · HTML5 ZIP</p>
                        <p className="font-mono text-xs text-[var(--text-dim)] mt-1">Max 50 MB</p>
                      </div>
                    </>
                  )}
                </div>
                {uploadError && <p className="mt-3 font-mono text-sm text-red-600">{uploadError}</p>}
              </div>
            )}
          ...
          <div className="p-8 flex flex-col justify-between gap-8">
            <div>
              <div className="mb-6">
                <p className="font-mono text-xs tracking-widest uppercase text-[var(--text-muted)] mb-1">Target</p>
                <p className="font-sans-ui text-xl font-600 text-black">Enter website URL</p>
              </div>
              ...
              <div>
                <input
                  ...
                  onKeyDown={(e) => { if (e.key === 'Enter' && canSubmit) handleDetect(); }}
                  onBlur={() => setUrlTouched(true)}
                  placeholder="https://example.com"
                  className={`input-field ${urlTouched && url && !isValidUrl ? 'error' : ''}`}
                />
                {urlTouched && url && !isValidUrl && (
                  <p className="mt-2 font-mono text-sm text-red-600">Enter a valid URL starting with https://</p>
                )}
              </div>

              <div className="mt-6 border-t border-[var(--line)] pt-6">
                <p className="font-sans-ui text-sm text-[var(--text-muted)] leading-relaxed">
                  We&apos;ll load the website in a headless browser, detect all ad slots using IAB standard dimensions and known ad selectors, then show you a preview of your creative in each slot.
                </p>
              </div>
            </div>
            ...
            <div>
              {!creative && (
                <p className="font-mono text-sm text-[var(--text-muted)] mb-4">← Upload a creative first</p>
              )}
              <button
              ...
              >
                {isNavigating ? (
                  <>
                    <span className="w-4 h-4 border border-white border-t-transparent animate-spin" style={{ borderRadius: 0 }} />
                    Starting scan…
                  </>
                ) : (
                  <>Detect Ad Slots →</>
